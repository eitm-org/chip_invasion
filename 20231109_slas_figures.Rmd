...
overlapping_orgs <- dplyr::intersect(fig2_counts$all_gfp_bottom[,1:43], fig2_counts_fp$all_gfp_bottom[,1:43]) %>% mutate(colors = '#0571b0',
         y = ((overlapping_orgs$position_x + abs(min(overlapping_orgs$position_x))) * (100/(abs(min(overlapping_orgs$position_x)) + max(overlapping_orgs$position_x)))),
         x = ((overlapping_orgs$position_y + abs(min(overlapping_orgs$position_y))) * (100/(abs(min(overlapping_orgs$position_y)) + max(overlapping_orgs$position_y)))),
         z=centroid_z)
#221 shared
unique_orgs <- dplyr::setdiff(fig2_counts$all_gfp_bottom[,1:43], fig2_counts_fp$all_gfp_bottom[,1:43])%>% mutate(colors = '#FF9800',
         y = ((unique_orgs$position_x + abs(min(unique_orgs$position_x))) * (100/(abs(min(unique_orgs$position_x)) + max(unique_orgs$position_x)))),
         x = ((unique_orgs$position_y + abs(min(unique_orgs$position_y))) * (100/(abs(min(unique_orgs$position_y)) + max(unique_orgs$position_y)))),
         z=centroid_z)

# unique_to_fp <- dplyr::setdiff(fig2_counts_fp$all_gfp_bottom[,1:43], fig2_counts$all_gfp_bottom[,1:43]) %>% mutate(colors = 'gray',
#          y = ((unique_to_fp$position_x + abs(min(unique_to_fp$position_x))) * (100/(abs(min(unique_to_fp$position_x)) + max(unique_to_fp$position_x)))),
#          x = ((unique_to_fp$position_y + abs(min(unique_to_fp$position_y))) * (100/(abs(min(unique_to_fp$position_y)) + max(unique_to_fp$position_y)))),
#          z=centroid_z)

orgs_with_color <- bind_rows(overlapping_orgs, unique_orgs, 
                             #unique_to_fp
                             )

#unique_to_fp <- dplyr::setdiff(fig2_counts_fp$all_gfp_bottom[,1:43], fig2_counts$all_gfp_bottom[,1:43])
#bro there are so many of you 
#cute_surface_maker_newbie(fig2_flat_plane$huvec_list, fig2_flat_plane$fit_list, chip_in = 1, bottom_gfp_df = fig2_counts$all_gfp_bottom)

#fig2_counts$gfp_per_chip_bottom
#fig2_counts_fp$gfp_per_chip_bottom
...
fig <- fig %>% layout(title = "3D Subplots",
         scene = list(domain=list(x=c(0.66,1),y=c(0,1)),
                      ...
                      aspectmode='cube'),
         scene2 = list(domain=list(x=c(0.0,0.5),y=c(0,1)),
                       xaxis=axx, yaxis=axx, zaxis=axx,
                       aspectmode='cube'),
...
#htmlwidgets::saveWidget(fig, file = "fig2_surface.html")
```
